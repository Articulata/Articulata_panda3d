...
class Server(QueuedConnectionManager):
    ...
    def tskReaderPolling(self,
                         regClass):  # This function listens for any data coming on already established functions
    ...
    def tskListenerPolling(self,
                           regClass):  # This Function checks to see if there are any new clients and adds their connection
        ...
        if self.cListener.newConnectionAvailable():
            ...
            if self.cListener.getNewConnection(self.rendezvous, self.netAddress,
                                               self.newConnection):
...
class PlayerReg(
    DirectObject):  # This class will hold anything that is related to regulating clients
    def __init__(self):
        self.PlayerList = []
        self.active = 0
    ...
    def sendInitialInfo(self, i, server):  # Initialize the new Player
        ...
        self.datagram.addString("init")  # specify to the assets that this is an init type packet
        self.datagram.addUint8(
            self.active)  # specify the player's number (not sure why this is here)
